...
const Members = () => {
  ...
  return (
    <div className="members-container">
      ...
      {/* Tabs */}
      <div className="tabs">
        <button className={tab === "list" ? "active-tab" : ""} onClick={() => setTab("list")}>
          List View
        </button>
        <button className={tab === "board" ? "active-tab" : ""} onClick={() => setTab("board")}>
          Board View
        </button>
      </div>
          ...
          {filteredMembers.map((member) => (
            <div key={member._id} className="member-card">
              ...
              <div className="member-role">
                <span
                  className={`role-badge ${["admin", "owner"].includes(member.role) ? "role-admin" : "role-member"}`}
                >
                  {member.role}
                </span>
        ...
        <div className="board-view">
          {filteredMembers.map((member) => (
            <div key={member._id} className="board-card">
              <div className="avatar">{member.name.substring(0, 2).toUpperCase()}</div>
              <h3>{member.name}</h3>
              <p>{member.email}</p>
              <p>{member.phone}</p>
